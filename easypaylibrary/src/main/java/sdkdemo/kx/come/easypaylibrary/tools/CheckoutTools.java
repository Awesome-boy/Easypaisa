package sdkdemo.kx.come.easypaylibrary.tools;

/**
 * Created by xief on 2016/12/6.
 */

public class CheckoutTools {

    //    public final static String HOST_TEST = "https://dev-pacypay2-gateway.ronhan.com";
    public final static String HOST_TEST = "https://sit-pacypay2-gateway.ronhan.com";
    public final static String HOST = "https://pg.pacypay.com";

    public final static String CHECKOUT_MERCHANT = "checkout_merchant";
    public final static String CHECKOUT_DEBUG = "checkout_debug";


    public final static String INFO = "CheckoutInfo";

    public final static String FORMAL_ENVIRONMENT = "Formal_Environment";
    public final static String SANDBOX_ENVIRONMENT = "Sandbox_Environment";


    public final static int INTERNATIONAL_CARD_TYPE = 117;
}
